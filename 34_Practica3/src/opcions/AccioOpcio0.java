package opcions;
/*
 * ==== CLASE OPCIO 0 (REQUERIDA PER LA PRACTICA) GRAFIC ====
 * 
...
public class AccioOpcio0 implements ActionListener {
    
    private LlistaActivitats llistMostrar;
     ...
     */
    public AccioOpcio0(LlistaActivitats llistaActivitats) {
        this.llistMostrar = llistaActivitats;
    }
    ...
    public void actionPerformed(ActionEvent evt) { 
        ...
        f.setVisible(false);
        JFrame ventana = new JFrame("Exercici PRAC 3");
        JPanel panelBotones = new JPanel(new GridLayout(2, 5, 10, 10));
        JPanel panelAtras = new JPanel(new GridLayout(1, 1, 10, 10));
        ...
        atras.addActionListener(e -> {
            ventana.dispose();
            f.setVisible(true);
            Log.logInfo("Se ha salido de la opcion 0");
        });
        ...
        mostrarNumActividadesDia(panelBotones);
        ventana.add(panelBotones, BorderLayout.CENTER);
        ventana.add(panelAtras, BorderLayout.SOUTH);
    }

    /** Mostra el nombre de activitats a cada butto. 
     * @param botones
     */
    private void mostrarNumActividadesDia(JPanel botones) {
        for (int i = 1; i < 11; i++) {
            ((JButton)botones.getComponent(i-1)).setText(String.valueOf(llistMostrar.diaConcret(i).getNElem()));
        }
    }

    /** Genera la ventana amb tots els botons. 
     * @param ventana
     * @param panelBotones
     */
    private void crearFrame(JFrame ventana, JPanel panelBotones) {
        ...
        JLabel titulo = new JLabel("Exercici PRAC 3");
    ...
    /** Ventana que apareixera si pulsem algun dels 10 buttons. 
     * @param ventana
     * @param botoAccionat
     */
    private void mostrarOpciones(JFrame ventana, JButton botoAccionat) {
        ...
        LlistaActivitats llistaDiaConcret = llistMostrar.diaConcret(dia);
        ...
        xerrades.addActionListener(e -> {
            if (hiHaXerrades) {
                mostrarXerrades(llistaDiaConcret);
            } else {
        ...
        visites.addActionListener(e -> {
            if (hiHaVisites) {
                mostrarVisites(llistaDiaConcret);
            } else {
        ...
        tallers.addActionListener(e -> {
            if (hiHaTallers) {
                mostrarTallers(llistaDiaConcret);
            } else {
    ...
    /** Verifiquem si existeix  algun tipus en concret en la llista de activitats. 
     * @param tipus
     * @param la
     * @return True/false
     */
    ...
    /** Mostra en un tauler totes les xerrades. 
     * @param llistaEntrada
     */
    public void mostrarXerrades(LlistaActivitats llistaEntrada) {
        int numXerrades = 0; 
    ...
    /** Mostra en un tauler totes les visites. 
     * @param llistaEntrada
     */
    public void mostrarVisites(LlistaActivitats llistaEntrada) {
        int numVisites = 0;
        for (int i = 0; i < llistaEntrada.getNElem(); i++) {
    ...
    /** Mostra en un tauler tots els tallers. 
     * @param llistaEntrada
     */
